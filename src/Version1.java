...
public class Version1 {
	...
	public Version1(File fXmlFile, ArrayList<String> event_ids, ArrayList<String> durations, ArrayList<String> times, ArrayList<String> rooms){
		...
		doc.getDocumentElement().normalize();
			...
			for (int i = 0; i < HighSchoolTimetableArchive.getLength(); i++) {
				...
				for(int j=0; j<event_ids.size(); j++){ 
					...
					time.setAttribute("Reference", times.get(0));
					Element resources = doc.createElement("Resources");
					event.appendChild(resources);
					Element resource = doc.createElement("Resource");
					resources.appendChild(resource);
					resource.setAttribute("Reference", rooms.get(0));					
				}
