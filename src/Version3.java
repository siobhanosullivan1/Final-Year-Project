...
public class Version3 {
	...
	public Version3(File fXmlFile, ArrayList<String> event_ids, ArrayList<String> durations, ArrayList<String> times, ArrayList<String> rooms){
		...
		doc.getDocumentElement().normalize();
			...
			for (int i = 0; i < HighSchoolTimetableArchive.getLength(); i++) {
				...
				for(int j=0; j<event_ids.size(); j++){ 
					int remaining_duration = Integer.parseInt(durations.get(j));
					while(remaining_duration>0){
						...
						int random_duration = rand.nextInt(remaining_duration) + 1;
						Element duration = doc.createElement("Duration");
						event.appendChild(duration);
						Node duration_names = doc.createTextNode(String.valueOf(random_duration));
						...
						int n = rand2.nextInt(times.size());
						...
						int n2 = rand3.nextInt(rooms.size());
						resource.setAttribute("Reference", rooms.get(n2));
						
						remaining_duration = remaining_duration-random_duration;
					}
				}
